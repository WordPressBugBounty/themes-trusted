...
function trusted_block_editor_dynamic_style( $css = array() ) {
	...
	$background_color = get_background_color();
	if ($background_color && $background_color != 'f7f7f7') {
		$css[] = '.editor-styles-wrapper{background: #'.esc_attr($background_color).';}';
	}
	...
	function trusted_woo_tabs() {
		...
		<section id="home-shop-section" class="section">
			...
			foreach ($tabs as $tab) {
				...
				if ($tab_active == 1) {
					if ( $tab_id == 'custom' ) {
						$tab_custom_page_id = get_theme_mod( 'tab_custom_page' );
						if ( $tab_custom_page_id ) {
							$tab_custom_post = get_post( $tab_custom_page_id );
							$tab_name = $tab_custom_post->post_title;
							if ( $tab_name == '' ) {
								$tab_name = $tab_custom_post->post_name;
							}
						echo '<li id="tab-title-'.$tab_id.'" class="'.$tab_id.'_tab"><a href="#tab-'.$tab_id.'">'.esc_html($tab_name).'</a></li>
						';
						}
			...
			foreach ($tabs as $tab) {
				...
				if ( $tab_id == 'custom' ) {
					if ( $tab_active == 1 ) {
						$tab_custom_page_id = get_theme_mod( 'tab_custom_page' );
						if ( $tab_custom_page_id ) {
						echo '<div id="tab-'.$tab_id.'" class="woocommerce-Tabs-panel woocommerce-Tabs-panel--'.$tab_id.' panel entry-content wc-tab">
						' . apply_filters( 'the_content', get_the_content( '', '', $tab_custom_page_id ) ) . '
						</div>
						';
						}
					}
					...
					if ( $tab_shortcode == 'featured_products' || $tab_shortcode == 'sale_products' ) {
						if ( $tab_shortcode == 'featured_products' ) {
							$tab_order = get_theme_mod( 'tab_featured_order', '' );
						} else {
							$tab_order = get_theme_mod( 'tab_sale_order', '' );
						}					
						if ( $tab_order == 'titleaz' ) {
							$orderby = ' orderby="title" order="ASC"';
						} elseif ( $tab_order == 'titleza' ) {
							$orderby = ' orderby="title" order="DESC"';
						} elseif ( $tab_order == 'datenew' ) {
							$orderby = ' orderby="date" order="DESC"';
						} elseif ( $tab_order == 'dateold' ) {
							$orderby = ' orderby="date" order="ASC"';
						} elseif ( $tab_order == 'pricehigh' ) {
							$orderby = ' orderby="price" order="DESC"';
						} elseif ( $tab_order == 'pricelow' ) {
							$orderby = ' orderby="price" order="ASC"';
						} elseif ( $tab_order == 'popular' ) {
							$orderby = ' orderby="popularity"';
						} elseif ( $tab_order == 'rating' ) {
							$orderby = ' orderby="rating"';
						} elseif ( $tab_order == 'random' ) {
					...
					if ($tab_active == 1) {
						echo '<div id="tab-'.$tab_id.'" class="woocommerce-Tabs-panel woocommerce-Tabs-panel--'.$tab_id.' panel entry-content wc-tab">'.do_shortcode('['.$tab_shortcode.' per_page="'.$per_page.'" columns="'.$columns.'"'.$orderby.']').'</div>
						';
					}
...
if(!function_exists( 'trusted_woo_home_tabs' )){
	function trusted_woo_home_tabs(){
		...
		$tabs['custom'] = array(
			...
			'callback' => 'trusted_custom',
			'shortcode'=> 'custom_page',
		);
